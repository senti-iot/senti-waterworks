	...
	*/
	return (
		<ItemG container style={{ width: 'auto' }} wrap={"nowrap"} alignItems={'center'} justify={'center'}>
			...
			<DayOption isSelected={period.menuId === 2} onClick={setDateTime(2, moment().subtract(1, 'd').startOf('day'), moment().subtract(7, 'day').startOf('day'), 2)}>{t('filters.dateOptions.7days')}</DayOption>
			<DayOption isSelected={period.menuId === 3} onClick={setDateTime(3, moment().subtract(1, 'd').startOf('day'), moment().startOf('month').startOf('day'), 2)}>{t('filters.dateOptions.month')}</DayOption>
			<DayOption isSelected={period.menuId === 4} onClick={setDateTime(4, moment().subtract(1, 'd').startOf('day'), moment().startOf('year').startOf('day'), 4)}>{t('filters.dateOptions.year')}</DayOption>
		</ItemG>
	)
}
